...
void MyClientMod::ClearSG()
{
    ...
    SG->RowCount = bNeedColumnNames+1;
    SG->ColCount = bNeedColumnNames+1;
    SG->ColWidths[bNeedColumnNames] = 20;// Ширина первой колонки. Нужна здесь на случай если управляющий поток не пойдет в RecieveTable()
    SG->ColWidths[0] = 12;
...
void MyClientMod::SendCommand(AnsiString sCommand, AnsiString sBase)
{
    ...
    if(GetIsConnectionOK())
    {
        ...
        if(nCode || TakeCodeFromServ())
        {
            ...
            nRowCount+= bNeedColumnNames;
            nColumnsCount+= bNeedColumnNames;
            short nMin = bNeedColumnNames+1;
            if(nRowCount<nMin) nRowCount = nMin;
            if(nColumnsCount<nMin) nColumnsCount = nMin;
...
void MyClientMod::RecieveTable(AnsiString sRequestID)
{
    ...
    while(true)
    {
        ...
        if(GetErrors(sReply))
            return;
        // Тут типа все ок, пишем инфу в таблицу


        // Если нужны имена колонок, тогда сначала увеличиваем nX, потом пишем результат,
        // а если не нужгы, тогда наоборот, сначала пишем потом увеличиваем
        nX+= bNeedColumnNames;

        if(nY == !bNeedColumnNames) // При первом проходе обнуляем ширину колонки
            SG->ColWidths[nX] = 0;
        SG->Cells[nX][nY-1+bNeedColumnNames] = sReply;
        CheckFieldWidth(sReply, nX);

        if(nX >= SG->ColCount-1 - !bNeedColumnNames)
        {
            nX = 0;
            nY+= bNeedColumnNames;
            if(nY > SG->RowCount-1) //Значит получена уже вся информация
            {
