...
void MySQLClient::AddErrorMessage(AnsiString sID)
{
    ...
    if(sID == "0")
        ...
        sMess = "Команда принята!";
    else if(sID == "6")
        sMess = "Неизвестная команда!";
    else
        sMess = sID;
...
//------------------------------------------------------------------------------
void MySQLClient::ReceiveClientReply(int nReqY, int nReqX)
{
    AnsiString sReply = Cli->CurrentReadBuffer();
    StringsContainer strCont;
    strCont.BreakString(sReply, "-||-");
    ...
    else if(strCont.Vector.size() == 1)
    {
        ...
        {
            if(nReqY == 0)// Значит информация придет с именами колонок
            {
                StringsContainer strCont2;
                strCont2.BreakString(strCont.Vector[0], "||");
                if(strCont2.Vector.size() == 2)
                {
                    SG->ColWidths[nReqX+1] = 0;
                    SG->Cells[nReqX+1][0] = strCont2.Vector[1];
                    CheckFieldWidth(strCont2.Vector[1], nReqX+1);
                    strCont.Vector[0] = strCont2.Vector[0];
                }
            }

            SG->Cells[nReqX+1][nReqY+1] = strCont.Vector[0];
            CheckFieldWidth(strCont.Vector[0], nReqX+1);
            nReqX++;
            if(nReqX >= SG->ColCount-1)
            {
                nReqX = 0;
                nReqY++;
                if(nReqY >= SG->RowCount-1)
                    nReqY = 0;
            }
            if(nReqY == 0 && nReqX == 0)//Значит получена уже вся информация
            {
            ...
            {
                Cli->Write(CodeString(sRequestID + "-||-" + IntToStr(nReqY+1) + "-||-" + IntToStr(nReqX)));
                ReceiveClientReply(nReqY, nReqX);
            }
        }
    }
    else
...
void MySQLClient::SendCommand(AnsiString sCommand)
{
    ...
    if(Cli->Connected())
    {
        ...
        sCommand = CodeString("kjubytrc-||-"+ sCommand);
        Cli->Write(sCommand);
        ReceiveReply();
    }
    else
...
//------------------------------------------------------------------------------
void MySQLClient::SendClientCommand(AnsiString sCommand)
{
    ClearSG();
    ...
    if(Cli->Connected())
    {
        Log("Connection... Success!");
        //Log("sCommand sent: " + sCommand);
        sCommand = CodeString("kjubytrc-||-"+ sCommand + "||ColNames");
        Cli->Write(sCommand);
        ReceiveClientReply();
    }
    else
...
//------------------------------------------------------------------------------
void MySQLClient::CheckFieldWidth(AnsiString sValue, int nX)
{
